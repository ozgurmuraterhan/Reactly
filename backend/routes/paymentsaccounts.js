...
let Paymentsaccounts = require("../models/paymentsaccounts.model");
let User = require("../models/user.model");

const title = "Payments Accounts";
const roleTitle = "paymentsaccounts";
...
router.route("/").get(passport.authenticate("jwt", { session: false }), (req, res, next) => {
   User.find({ username: req.user.username }).then((data) => {
      ...
      if (rolesControl[roleTitle + "list"]) {
         ...
         Paymentsaccounts.find({ "created_user.id": `${req.user._id}` })
            .then((data) => {
               res.json(data);
            })
            .catch((err) =>
...
router.route("/:id").get(passport.authenticate("jwt", { session: false }), (req, res, next) => {
   User.find({ username: req.user.username }).then((data) => {
      const rolesControl = data[0].role;
      if (rolesControl[roleTitle + "list"]) {
         Paymentsaccounts.findById(req.params.id)
            .then((data) => res.json(data))
            .catch((err) =>
               res.status(400).json({
                  messagge: "Error: " + err,
...
router.route("/:id").delete(passport.authenticate("jwt", { session: false }), (req, res) => {
   User.find({ username: req.user.username }).then((data) => {
      ...
      if (rolesControl[roleTitle + "delete"]) {
         Paymentsaccounts.findByIdAndDelete(req.params.id)
            .then((data) =>
               res.json({
...
router.route("/:id").post(passport.authenticate("jwt", { session: false }), (req, res, next) => {
   User.find({ username: req.user.username }).then((data) => {
      ...
      if (rolesControl[roleTitle + "edit"]) {
         Paymentsaccounts.findByIdAndUpdate(req.params.id, req.body)
            .then(() =>
               res.json({
         ...
         Paymentsaccounts.findOneAndUpdate(
            {
               _id: req.params.id,
               "created_user.id": `${req.user._id}`,
            },
